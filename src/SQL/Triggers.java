...
public class Triggers extends Base{
    ...
    public Triggers(Connection c) {
        ...
        this.loggerDAnnotations = "CREATE OR REPLACE TRIGGER loggerDAnnotations\n" +
                "AFTER INSERT OR UPDATE ON Annoter\n" +
                "FOR EACH ROW\n" +
                "declare\n" +
                "    v_log_chercheurs NUMBER;\n" +
                "begin\n" +
                "    -- On vérifie si la table log_chercheurs déjà\n" +
                "    select COUNT(*) into v_log_chercheurs\n" +
                "    from user_tables\n" +
                "    where table_name = 'LOG_CHERCHEURS';\n" +
                "\n" +
                "    -- Si elle existe pas, on la crée\n" +
                "    if v_log_chercheurs = 0 then\n" +
                "        EXECUTE IMMEDIATE\n" +
                "            'create table LOG_CHERCHEURS (\n" +
                "                id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY,\n" +
                "                utilisateur varchar2(500),\n" +
                "                date date,\n" +
                ...
                "    end if;\n" +
                "\n" +
                "    -- puis on enregistre dans la table l'action faite par un utilisateur\n" +
    ...
    public String triggerCreer(boolean creer, String nomTrigger) throws SQLException {
        ...
        if(creer){
            // Puis s'il existe déjà, on renvoie qu'il existe déjà
            if(existe)
